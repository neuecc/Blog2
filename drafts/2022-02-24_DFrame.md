...
![image](https://user-images.githubusercontent.com/46207/155888611-726bb9f0-0d9a-4124-9e8e-55380889d1cd.png)
...
と、いうわけで、WorkloadのExecuteAsyncにコードを書くのが基本です。ExecuteAsync前の準備用としてSetupAsync、後始末としてTeardownAsyncもあります。単純なHTTP Getを書くとこなります。
...
public class SampleWorkload2 : Workload
{
    readonly HttpClient httpClient;
    readonly string url;

    // HttpClient is from DI, URL is passed from Web UI
    public SampleWorkload2(HttpClient httpClient, string url)
    {
...
![image](https://user-images.githubusercontent.com/46207/155886846-e9c0eafb-51db-4c90-abd1-369df2e59ac1.png)
...
![image](https://user-images.githubusercontent.com/46207/155888098-e3b5d0bd-8d21-4bc5-a355-27d1f62715b3.png)
...
abが、6287 req/sec、k6が125619 req/sec、DFrameが207634 req/secです。abは、厳しい、厳しい……。さすがにもっと出るはずと思っているんですが、私の環境(Windows)だと昔からこんな感じなので、性能的には信用できないかな、と。DFrameの場合Concurrencyにまだ余裕があって、増やすとまだまだ伸びたのですが、k6は割と頭打ちでした。
